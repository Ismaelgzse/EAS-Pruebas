...
import model.game.GameError;
import model.game.Player;
import model.game.Suggestion;
import model.tile.Position;
import model.tile.Room;
...
import gameLogic.Configs;

/**
 * A GUI client for Cluedo model.game.
 * 
 * @author Hector
 *
 */
...
public class GUIClient extends JFrame {
    ...
    /**
     * Pop up a dialog to setup how many players and how many dices are used in model.game
     */
    ...
    /**
     * Initialise the model.game with the given number of players and number of dices
     * 
     * @param numPlayers
     *            --- how many players
     * @param numDices
     *            --- how many dices are used in model.game
     */
    ...
    /**
     * This method construct the in-model.game GUI, and let the model.game begin.
     */
    public void startGame() {

        // first let's finish initialising the model.game
        game.decideWhoMoveFirst();
        game.creatSolution();
        game.dealCard();

        // remove the welcome screen, and load into the model.game interface
        this.remove(window);
        window = new JPanel();
        window.setLayout(new BoxLayout(window, BoxLayout.X_AXIS));

        // now make the left panel, which is model.game board
    ...
    /**
     * This method updates model.game board and player panel display according to the model
     * (model.game).
     */
    public void update() {
        if (game.isGameRunning()) {
            ...
            // model.game stopped, we must have a winner
     ...
     * pop up a option panel to show if he is the winner or he loses the model.game.
     * 
     * @param accusation
    ...
    public void makeAccusation(Suggestion accusation) {
        ...
        if (isCorrect) {
            ...
            // start a new model.game or quit
            if (choice == JOptionPane.OK_OPTION) {
                setupNumPlayers();
            } else {
                System.exit(0);
    ...
    public void movePlayer(Character character, Position position) {
        ...
        if (position instanceof Tile) {
            // Tile model.tile = (Tile) position;
            // characterTokens[character.ordinal()].moveToTile(model.tile);
     ...
     *            --- the character to be moved
     * @param roomTile
     *            --- which room to move into, and on which model.tile is this token put
     */
    ...
    /**
     * Get the model.game board
     * 
     * @return --- the model.game board
     */
    ...
    /**
     * Is the model.game run on easy mode?
     * 
     * @return --- true if the model.game run on easy mode, or false if not.
     */
    ...
    /**
     * Set the model.game to easy mode (so that the model.game will remember clues for
     * player...cheating).
     * 
     * @param isEasyMode
    ...
    /**
     * Whether model.game has a winner (i.e. model.game end)
     * 
     * @return --- true if model.game is still running, there is no winner yet; false if not.
     */
     ...
     * order, which is: north model.tile -> east model.tile -> south model.tile -> west model.tile -> room if
     ...
     * in this list. In particular, a model.tile on which has another player standing will not
     * be added in.<br>
     * <br>
    ...
    /**
     * Main method to start the model.game.
     * 
     * @param args
     *            --- who cares it in GUI?
     */
